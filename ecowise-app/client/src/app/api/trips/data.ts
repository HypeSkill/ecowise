...
const baseTrips: Trip[] = [
  ...
  },
  {
    ...
    plan: [
      {
        ...
        source: "Mumbai",
        destination: "Goa",
        serviceNumber: "Konkan Night",
        departureTime: "2026-04-10T21:30:00",
        arrivalTime: "2026-04-11T08:50:00",
        cost: 1400,
        durationHrs: 11.5,
        layover: null,
        bufferMins: 30,
        bufferNote: "Boarding buffer",
        availability: "Limited",
      ...
      },
      {
        ...
        serviceNumber: null,
        departureTime: "2026-04-12T07:00:00",
        arrivalTime: "2026-04-12T10:00:00",
        cost: 400,
        durationHrs: 3,
        layover: null,
        bufferMins: null,
      ...
      },
      {
        ...
        destination: "Mumbai",
        serviceNumber: "Konkan Night",
        departureTime: "2026-04-14T20:40:00",
        arrivalTime: "2026-04-15T08:00:00",
        cost: 1400,
        durationHrs: 11.4,
        layover: null,
        bufferMins: 30,
    ...
    userID: "demo-user",
    createdAt: now,
    travelSelection: { outboundId: "Konkan Night", returnId: "Konkan Night", outboundCost: 1400, returnCost: 1400 },
    sideLocations: [{ name: "Divar Island", days: 1, budget: 2200 }],
  },
];
...
export const deleteTrip = (id: string) => {
  const nextTrips = trips.filter((trip) => trip._id !== id);
  const deleted = nextTrips.length !== trips.length;
  trips = nextTrips;
  return deleted;
};

export const generateTripsForPrompt = (prompt: string) => {
  const cleanPrompt = prompt.trim().slice(0, 160);
  const promptLower = cleanPrompt.toLowerCase();
  const timestamp = new Date().toISOString();

  const matches = baseTrips.filter((trip) => {
    const from = trip.from.toLowerCase();
    const to = trip.to.toLowerCase();
    return promptLower.includes(from) || promptLower.includes(to);
  });

  const selectedTrips = matches.length > 0 ? matches : baseTrips;

  return selectedTrips.map((trip, index) => {
    const suffix = cleanPrompt ? `Prompt: ${cleanPrompt}` : "Prompt: eco-friendly itinerary";
    return {
      ...trip,
      _id: `mock-plan-${index + 1}-${Date.now()}`,
      createdAt: timestamp,
      plan_rationale: trip.plan_rationale ? `${trip.plan_rationale} ${suffix}` : suffix,
    };
  });
};
