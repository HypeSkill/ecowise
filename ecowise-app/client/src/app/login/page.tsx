...
export default function LoginPage() {
  ...
  const handleDemoLogin = () => {
    try { localStorage.setItem('sessionToken', 'demo-session'); } catch (e) {}
    setToast({ type: 'success', message: 'Signed in as demo user. Redirecting…' });
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((normalizedEmail === "demo" || normalizedEmail === "demo@ecowise.ai") && password === "1234") {
        try { localStorage.setItem('sessionToken', 'demo-session'); } catch (e) {}
        setToast({ type: 'success', message: 'Signed in as demo user. Redirecting…' });
        setTimeout(() => router.push('/dashboard'), 300);
        return;
    ...
    <main className="min-h-screen bg-site text-site">
      ...
      <section className="flex justify-center px-4 py-10">
        <div className="max-w-md w-full animate-fade-up">
          ...
          <p className="text-center text-sm text-muted mb-4">Demo access: user <strong>demo</strong> · password <strong>1234</strong></p>
          <button
            type="button"
            onClick={handleDemoLogin}
            className="w-full h-10 rounded-xl bg-card text-site text-sm font-semibold shadow-sm mb-4"
          >
            Skip login (demo)
          </button>
