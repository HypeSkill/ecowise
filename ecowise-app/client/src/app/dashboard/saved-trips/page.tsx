  ...
  useEffect(() => {
    const fetchSavedTrips = async () => {
      ...
      try {
        ...
        const res = await fetch(`${apiBase}/api/trips?userID=${uid}`, { headers, credentials: 'include' });
              ...
              {loading ? (
                ...
                savedTrips.map((trip, idx) => (
                  <div key={getPlanKey(trip, idx)} className="mb-8 p-6 rounded-lg bg-card shadow-lg animate-fade-up" style={{ animationDelay: `${idx * 80}ms` }}>
                    {demoMode && (
                      <div className="mb-3 text-xs font-semibold text-emerald-600">Demo mode • synthetic saved trip</div>
                    )}
                    ...
                    <div className="flex px-4 py-3 justify-between mt-4">
                      ...
                      <button
                        ...
                        className="flex items-center justify-center rounded-xl h-10 px-4 btn-primary text-white text-sm font-bold shadow-md"
                      >
                        Start Over
                      </button>
                    </div>
